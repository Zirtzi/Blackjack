...
def dealer_play(deck , dealer_hand, player_hand):
    ...
    if value_check("<", Dealer_Total, 17):
        ...
        if needs_to_play:
            ...
            print("\n" "\033[1;33;40m{player_hand.name}\033[0m has busted and therefore the dealer does not need to play their hand.")
        else:
            pass
        ...
        if needs_to_play:
            ...
            print("\n" "\033[1;33;40m{player_hand.name}\033[0m has busted and the dealer does not need to play their hand.")
        dealer_hand.Show_Hand(1, "final")
    elif value_check(">", Dealer_Total, 21):
    ...
    if all_over_21:
        if isinstance(player_hand, list):
            print("\n" "\033[1;33;40m{player_hand.name}\033[0m has busted on all hands therefore the dealer has won both hands."
...
def hit_stay_double_down(deck, dealer_hand, player_hand):
    ...
    while response not in ["h", "s", "d"] and value_check("<", Player_Total, 21):
        ...
        if response.lower() == "h":
            ...
            checking_bank = Player_Bank
            checking_bank -= player_hand.hand_wager
            if float(checking_bank) >= float(player_hand.hand_wager):
...
def play_hand(deck, dealer_hand, player_hand):
    ...
    while player_hand.hand_wager == 0:
        ...
        if player_hand.hand_wager > Player_Bank:
            print("\n" f"You have entered a wager of \033[1;31;40m{player_hand.hand_wager}\033[0m that is more than your bank"
                  f" of \033[1;31;40m{Player_Bank}\033[0m. Please re-enter your wager." "\n" )
            player_hand.hand_wager = 0
            continue
        elif player_hand.hand_wager <= 0:
            print("\n" f"Your wager of \033[1;31;40m{player_hand.hand_wager}\033[0m is less than or equal to zero. Please "
                  f"re-enter a positive value for a wager." "\n" )
            player_hand.hand_wager = 0
            continue
    ...
    if dealer_hand.cards[1].rank == "Ace":
        ...
        while insurance_choice not in ["y", "n"]:
            ...
            if insurance_choice.lower() == "y":
                ...
                time.sleep(1)
                print("\n" "\033[1;33;40m{player_hand.name}\033[0m has chosen to not buy insurance. We will now check for if the dealer"
                      " has blackjack.")
                if check_for_blackjack(dealer_hand.cards) and check_for_blackjack(player_hand.cards):
                    time.sleep(1)
                    print("\n" "Both players have blackjack, it is a \033[1;31;40mpush\033[0m.")
...
def player_same_rank_check(deck, player_hand):
    ...
    if check_same_rank_in_hand(player_hand.cards, "Ace"):
        ...
        checking_bank = Player_Bank
        checking_bank -= player_hand.hand_wager
        if value_check(">=", float(checking_bank), float(player_hand.hand_wager)):
            aces_response = ""
        ...
        time.sleep(1)
        if float(Player_Bank) >= 2.0*float(player_hand.hand_wager):
            same_rank_response = ""
            while same_rank_response not in ["y", "n"]:
                ...
                if same_rank_response.lower() == "y":
                    ...
                    checking_bank = Player_Bank
                    checking_bank -= player_hand_1.hand_wager + player_hand_2.hand_wager
                            ...
                            while value_check("<=", split_counter, 3) and check_same_rank_in_hand(checking_hand.cards):
                                ...
                                while split_again_choice not in ["y", "n"]:
                                    ...
                                    if split_again_choice.lower() == "y":
                                        ...
                                        checking_bank -= new_player_hand_2.hand_wager
                                        break
                                ...
                                if check_same_rank_in_hand(checking_hand.cards) and split_again_choice.lower() == "y":
                                    ...
                                    value_check(">=", float(checking_bank), float(player_hand.hand_wager)):
                                    ...
                                    value_check("<", float(checking_bank), float(player_hand.hand_wager)):
                                        ...
                                        elif value_check("<", float(checking_bank), float(player_hand.hand_wager)):
                                            print("\n" f"Your current bank balance"
                                                  f" of \033[1;31;40m{checking_bank}\033[0m is not greater than your"
